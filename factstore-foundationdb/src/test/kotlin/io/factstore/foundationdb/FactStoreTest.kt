...
class FactStoreTest {
    ...
    fun testIsolationOfFactStoreInstances(): Unit = runBlocking {
        ...
        val factStore1 = buildFdbFactStore(clusterFilePath = clusterFilePath)
        val factStore2 = buildFdbFactStore(clusterFilePath = clusterFilePath)

        val factstoreId1 = FactStoreId.generate()
        val factstoreId2 = FactStoreId.generate()
        ...
        factStore1.append(factstoreId1, fact1)
        factStore2.append(factstoreId2, fact2)

        assertThat(factStore1.existsById(factstoreId1, fact1.id)).isTrue()
        assertThat(factStore1.existsById(factstoreId1, fact2.id)).isFalse()

        assertThat(factStore2.existsById(factstoreId2, fact1.id)).isFalse()
        assertThat(factStore2.existsById(factstoreId2, fact2.id)).isTrue()

        // instantiating a third instance with the same name as factstore1 should point to the same logical fact store
        val factStore3 = buildFdbFactStore(clusterFilePath = clusterFilePath)
        assertThat(factStore3.existsById(factstoreId1, fact1.id)).isTrue()
        assertThat(factStore3.existsById(factstoreId1, fact2.id)).isFalse()
    }
