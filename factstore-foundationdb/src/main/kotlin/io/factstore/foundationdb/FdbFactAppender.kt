...
class FdbFactAppender(
    ...
    Think about introducing a FdbAppendContext data class or similar
    to store / reference all data needed for handling an append request
    - current transaction
    - the append request itself
    - subspaces?

    This should solve the problem of propagating a lot of values in methods.
    Instead, we pass only one object (the append context)

    Opt in for experimental feature: context parameters!
    It will become stable with Kotlin version 2.4
